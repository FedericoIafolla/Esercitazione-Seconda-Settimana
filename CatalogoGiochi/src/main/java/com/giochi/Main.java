...
public class Main {
    ...
    private static void cercaGiocoPerID(Collezione collezione, Scanner scanner) {
        eseguiOperazione(scanner, "Inserisci ID gioco (premi 'Esc' per tornare indietro): ", idCerca -> {
    ...
    private static void cercaGiochiPerPrezzo(Collezione collezione, Scanner scanner) {
        eseguiOperazione(scanner, "Inserisci prezzo (premi 'Esc' per tornare indietro): ", prezzoStr -> {
    ...
    private static void cercaGiochiDaTavoloPerGiocatori(Collezione collezione, Scanner scanner) {
        eseguiOperazione(scanner, "Inserisci numero di giocatori (premi 'Esc' per tornare indietro): ", numGiocatoriStr -> {
    ...
    private static void modificaGioco(Collezione collezione, Scanner scanner, String azione) {
        eseguiOperazione(scanner, "Inserisci ID gioco da " + (azione.equals("rimuovi") ? "rimuovere" : "aggiornare") + " (premi 'Esc' per tornare indietro): ", idGioco -> {
            if (azione.equals("rimuovi")) {
                if (collezione.cercaPerID(idGioco).isPresent()) {
                    collezione.rimuoviGioco(idGioco);
                    System.out.println("Gioco rimosso con successo.");
                } else {
                    System.out.println("Nessun gioco trovato con ID: " + idGioco);
                }
            } else {
    ...
    private static void aggiornaGioco(Collezione collezione, Scanner scanner, String idAggiorna) {
        ...
        if (giocoEsistente instanceof GiocoDaTavolo) {
            ...
            int durataGioco = leggiDatiInt(scanner, "Inserisci durata del gioco (in ore): ");
            int genereIndex = leggiDatiInt(scanner, "Inserisci genere (0. AZIONE 1. AVVENTURA 2. HORROR 3. FPS 4. TPS): ");
    ...
    private static void aggiungiVideogioco(Collezione collezione, Scanner scanner, String id, String titolo, int anno, double prezzo) {
        String piattaforma = leggiDati(scanner, "Inserisci piattaforma: ");
        int durataGioco = leggiDatiInt(scanner, "Inserisci durata del gioco (in ore): ");
        int genereIndex = leggiDatiInt(scanner, "Inserisci genere (0-4): ");
        Genere genere = Genere.values()[genereIndex];
        Videogioco nuovoVideogioco = new Videogioco(id, titolo, anno, prezzo, piattaforma, durataGioco, genere);
    ...
    private static void mostraListaGiochi(Collezione collezione) {
        System.out.println("Lista di giochi nella collezione:");
